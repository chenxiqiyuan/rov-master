...
void AUV_Depth_Control(Rocker_Type *rc)
{
	...
	{ //AUV深度控制位数字量
		...
		case Dive:
			if (Total_Controller.High_Position_Control.Control_OutPut < 450)
			{ //超过输出范围 停止累积
				Expect_Depth += 3;
			}
...
void ROV_Depth_Control(Rocker_Type *rc)
{
	...
	if (FOUR_AXIS == VehicleMode) //ROV由 摇杆模拟量控制
	{
		...
		{
			if (Total_Controller.High_Position_Control.Control_OutPut < 300)
			{ //超过输出范围 停止累积
